...
let words = ref<Word[] | null>(props.paginator.data)


console.log(props.paginator)
//console.log(pagination_location)
//console.log(props.paginator)
//console.log(window)
//onMounted(()=>{})
//ids selected by checkbox below
const ids = ref([])
let message = ref('')
...
const del = (id:number) => {
    useForm(pagination_location).delete(route('words.destroy', id.toString()), {
        preserveScroll:true,
...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="mt-8">
            <table class="w-100">
                <tbody>
                    <tr v-for="word in words" :key="word.id" class="flex w-100 pt-2 pb-2 justify-between">
                        <td class=" w-1/2">
                                ...
                                <Link   :href="route('words.edit', word.id.toString())" class="mr-4"
                                        :data="pagination_location"
                                        :id="word.id">
                                        <span class=" text-4xl mt-1">{{word.word}}</span>
                                </Link>
                        </td>
                        <td class="opacity-25">

                            <button  @click="del(word.id)">
        ...
        <!-- <div v-if="paginator.meta.links.length > 2">
            <div class="flex flex-wrap -mb-1 mt-4">
            <template v-for="(link, key) in paginator.meta.links">
